package app;

public class Constants {
    public static int INACTIVE_CARD_WIDTH = 35, INACTIVE_CARD_HEIGHT = 56;
    public static int ACTIVE_CARD_WIDTH = 100, ACTIVE_CARD_HEIGHT = 160;
    public static int INACTIVE_CARD_WIDTH_WITH_PADDING = 45, INACTIVE_CARD_HEIGHT_WITH_PADDING = 60;
    public static int ACTIVE_CARD_WIDTH_WITH_PADDING = 110, ACTIVE_CARD_HEIGHT_WITH_PADDING = 170;
    public static int DRAW_DECK_X = -800;
    public static int DRAW_DECK_Y = 0;
    public static int DISCARD_DECK_X = -500;
    public static int DISCARD_DECK_Y = 0;
    public static int INACTIVE_MATRIX_WIDTH_WITH_PADDING = 210;
    public static int INACTIVE_MATRIX_Y = 200;
    public static int DRAWN_CARD_X = -200;
    public static int DRAWN_CARD_Y = 350;

    public static int ZERO_X = -960;
    public static int ZERO_Y = -450;
    public static int CONTINUE_BUTTON_X = 900;
    public static int CONTINUE_BUTTON_Y = 400;
}
